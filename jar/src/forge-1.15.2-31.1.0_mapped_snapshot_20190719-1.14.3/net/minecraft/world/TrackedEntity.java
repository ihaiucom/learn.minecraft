...
public class TrackedEntity {
   ...
   public void tick() {
      ...
      if (this.updateCounter % this.updateFrequency == 0 || this.trackedEntity.isAirBorne || this.trackedEntity.getDataManager().isDirty()) {
         ...
         if (this.trackedEntity.isPassenger()) {
            l = MathHelper.floor(this.trackedEntity.rotationYaw * 256.0F / 360.0F);
            k1 = MathHelper.floor(this.trackedEntity.rotationPitch * 256.0F / 360.0F);
            boolean flag2 = Math.abs(l - this.encodedRotationYaw) >= 1 || Math.abs(k1 - this.encodedRotationPitch) >= 1;
            if (flag2) {
               this.packetConsumer.accept(new SEntityPacket.LookPacket(this.trackedEntity.getEntityId(), (byte)l, (byte)k1, this.trackedEntity.onGround));
            ...
            boolean flag3 = vec3d.lengthSquared() >= 7.62939453125E-6D;
         ...
         l = MathHelper.floor(this.trackedEntity.getRotationYawHead() * 256.0F / 360.0F);
         if (Math.abs(l - this.encodedRotationYawHead) >= 1) {
            this.packetConsumer.accept(new SEntityHeadLookPacket(this.trackedEntity, (byte)l));
            this.encodedRotationYawHead = l;
         }
   ...
   public void untrack(ServerPlayerEntity p_219454_1_) {
      ...
      ForgeEventFactory.onStopEntityTracking(this.trackedEntity, p_219454_1_);
   }

   public void track(ServerPlayerEntity p_219455_1_) {
      ServerPlayNetHandler var10001 = p_219455_1_.connection;
      this.sendSpawnPackets(var10001::sendPacket);
      this.trackedEntity.addTrackingPlayer(p_219455_1_);
      p_219455_1_.addEntity(this.trackedEntity);
      ForgeEventFactory.onStartEntityTracking(this.trackedEntity, p_219455_1_);
   }
   ...
   public void sendSpawnPackets(Consumer<IPacket<?>> p_219452_1_) {
      ...
      if (this.trackedEntity instanceof LivingEntity) {
         EquipmentSlotType[] var9 = EquipmentSlotType.values();
         int var12 = var9.length;

         for(int var6 = 0; var6 < var12; ++var6) {
            EquipmentSlotType equipmentslottype = var9[var6];
      ...
      if (this.trackedEntity instanceof LivingEntity) {
         ...
         Iterator var13 = livingentity.getActivePotionEffects().iterator();

         while(var13.hasNext()) {
            EffectInstance effectinstance = (EffectInstance)var13.next();
   ...
   private void updateEncodedPosition() {
      this.encodedPosX = SEntityPacket.func_218743_a(this.trackedEntity.func_226277_ct_());
      this.encodedPosY = SEntityPacket.func_218743_a(this.trackedEntity.func_226278_cu_());
      this.encodedPosZ = SEntityPacket.func_218743_a(this.trackedEntity.func_226281_cx_());
   }
