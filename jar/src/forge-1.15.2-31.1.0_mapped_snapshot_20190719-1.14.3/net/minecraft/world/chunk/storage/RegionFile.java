...
public class RegionFile implements AutoCloseable {
   ...
   private int forgeGetRealLength(int p_forgeGetRealLength_1_, int p_forgeGetRealLength_2_) throws IOException {
      int chunkX = p_forgeGetRealLength_1_ & 31;
      int chunkZ = p_forgeGetRealLength_1_ >> 5 & 31;
      ByteBuffer header = ByteBuffer.allocate(5);
      this.dataFile.read(header, (long)(p_forgeGetRealLength_2_ * 4096));
      header.flip();
   ...
   public RegionFile extractLargeChunks(ChunkPos p_extractLargeChunks_1_) throws IOException {
      ChunkPos regionBase = new ChunkPos(p_extractLargeChunks_1_.getRegionCoordX() * 32, p_extractLargeChunks_1_.getRegionCoordZ() * 32);

      for(int index = 0; index < 1024; ++index) {
         int offset = this.offsets.get(index);
         if (func_227131_a_(offset) == 255) {
            offset = func_227142_b_(offset);
            ChunkPos chunk = new ChunkPos(regionBase.x + (index & 31), regionBase.z + (index >> 5 & 31));
            ByteBuffer header = ByteBuffer.allocate(5);
            this.dataFile.read(header, (long)(offset * 4096));
            header.flip();
            if (header.remaining() < 5) {
               ...
               if (sectors > 255 && !func_227130_a_(version)) {
                  ...
                  this.dataFile.read(data, (long)(offset * 4096));
                  data.flip();
   ...
   public synchronized DataInputStream func_222666_a(ChunkPos p_222666_1_) throws IOException {
      ...
      if (i == 0) {
         ...
         if (bytebuffer.remaining() < 5) {
            ...
            if (i1 == 0) {
               field_227122_a_.warn("Chunk {} is allocated, but stream is missing", p_222666_1_);
               return null;
            } else {
               int j1 = i1 - 1;
   ...
   private DataInputStream func_227133_a_(ChunkPos p_227133_1_, byte p_227133_2_) throws IOException {
      ...
      if (!Files.isRegularFile(path, new LinkOption[0])) {
         field_227122_a_.error("External chunk path {} is not file", path);
         return null;
      } else {
