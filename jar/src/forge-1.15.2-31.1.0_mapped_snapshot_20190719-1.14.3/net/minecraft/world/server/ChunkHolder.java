...
public class ChunkHolder {
   public static final Either<IChunk, ChunkHolder.IChunkLoadingError> MISSING_CHUNK;
   public static final CompletableFuture<Either<IChunk, ChunkHolder.IChunkLoadingError>> MISSING_CHUNK_FUTURE;
   public static final Either<Chunk, ChunkHolder.IChunkLoadingError> UNLOADED_CHUNK;
   private static final CompletableFuture<Either<Chunk, ChunkHolder.IChunkLoadingError>> UNLOADED_CHUNK_FUTURE;
   private static final List<ChunkStatus> CHUNK_STATUS_LIST;
   ...
   public ChunkHolder(ChunkPos p_i50716_1_, int p_i50716_2_, WorldLightManager p_i50716_3_, ChunkHolder.IListener p_i50716_4_, ChunkHolder.IPlayerProvider p_i50716_5_) {
      this.field_219312_g = new AtomicReferenceArray(CHUNK_STATUS_LIST.size());
      ...
      this.field_219315_j = CompletableFuture.completedFuture((IChunk)null);
      this.changedBlockPositions = new short[64];
   ...
   public CompletableFuture<Either<IChunk, ChunkHolder.IChunkLoadingError>> func_219301_a(ChunkStatus p_219301_1_) {
      CompletableFuture<Either<IChunk, ChunkHolder.IChunkLoadingError>> completablefuture = (CompletableFuture)this.field_219312_g.get(p_219301_1_.ordinal());
   ...
   public Chunk func_219298_c() {
      ...
      Either<Chunk, ChunkHolder.IChunkLoadingError> either = (Either)completablefuture.getNow((Either)null);
      return either == null ? null : (Chunk)either.left().orElse((Chunk)null);
   }
   ...
   public ChunkStatus func_219285_d() {
      for(int i = CHUNK_STATUS_LIST.size() - 1; i >= 0; --i) {
         ChunkStatus chunkstatus = (ChunkStatus)CHUNK_STATUS_LIST.get(i);
         CompletableFuture<Either<IChunk, ChunkHolder.IChunkLoadingError>> completablefuture = this.func_219301_a(chunkstatus);
         if (((Either)completablefuture.getNow(MISSING_CHUNK)).left().isPresent()) {
            return chunkstatus;
         }
      }
   ...
   public IChunk func_219287_e() {
      for(int i = CHUNK_STATUS_LIST.size() - 1; i >= 0; --i) {
         ...
         if (!completablefuture.isCompletedExceptionally()) {
            Optional<IChunk> optional = ((Either)completablefuture.getNow(MISSING_CHUNK)).left();
   ...
   public void markBlockChanged(int p_219279_1_, int p_219279_2_, int p_219279_3_) {
      Chunk chunk = this.func_219298_c();
      if (chunk != null) {
         this.blockChangeMask |= 1 << (p_219279_2_ >> 4);
         ...
         if (this.changedBlocks == this.changedBlockPositions.length) {
            this.changedBlockPositions = Arrays.copyOf(this.changedBlockPositions, this.changedBlockPositions.length << 1);
         }
   ...
   public void markLightChanged(LightType p_219280_1_, int p_219280_2_) {
      Chunk chunk = this.func_219298_c();
      if (chunk != null) {
         chunk.setModified(true);
         if (p_219280_1_ == LightType.SKY) {
            this.skyLightChangeMask |= 1 << p_219280_2_ - -1;
         } else {
            this.blockLightChangeMask |= 1 << p_219280_2_ - -1;
         }
      }

   }

   public void sendChanges(Chunk p_219274_1_) {
      if (this.changedBlocks != 0 || this.skyLightChangeMask != 0 || this.blockLightChangeMask != 0) {
         World world = p_219274_1_.getWorld();
         if (this.changedBlocks >= (Integer)ForgeConfig.SERVER.clumpingThreshold.get()) {
            this.boundaryMask = -1;
         }
         ...
         if (this.skyLightChangeMask != 0 || this.blockLightChangeMask != 0) {
            this.sendToTracking(new SUpdateLightPacket(p_219274_1_.getPos(), this.lightManager, this.skyLightChangeMask & ~this.boundaryMask, this.blockLightChangeMask & ~this.boundaryMask), true);
            i1 = this.skyLightChangeMask & this.boundaryMask;
            k1 = this.blockLightChangeMask & this.boundaryMask;
            if (i1 != 0 || k1 != 0) {
               this.sendToTracking(new SUpdateLightPacket(p_219274_1_.getPos(), this.lightManager, i1, k1), false);
            }
         ...
         if (this.changedBlocks == 1) {
            i1 = (this.changedBlockPositions[0] >> 12 & 15) + this.pos.x * 16;
            k1 = this.changedBlockPositions[0] & 255;
            l1 = (this.changedBlockPositions[0] >> 8 & 15) + this.pos.z * 16;
            BlockPos blockpos = new BlockPos(i1, k1, l1);
         ...
         } else if (this.changedBlocks >= (Integer)ForgeConfig.SERVER.clumpingThreshold.get()) {
            this.sendToTracking(new SChunkDataPacket(p_219274_1_, this.blockChangeMask), false);
         } else if (this.changedBlocks != 0) {
            this.sendToTracking(new SMultiBlockChangePacket(this.changedBlocks, this.changedBlockPositions, p_219274_1_), false);

            for(i1 = 0; i1 < this.changedBlocks; ++i1) {
               k1 = (this.changedBlockPositions[i1] >> 12 & 15) + this.pos.x * 16;
               l1 = this.changedBlockPositions[i1] & 255;
   ...
   private void sendTileEntity(World p_219305_1_, BlockPos p_219305_2_) {
   ...
   private void sendToTracking(IPacket<?> p_219293_1_, boolean p_219293_2_) {
      this.playerProvider.getTrackingPlayers(this.pos, p_219293_2_).forEach((p_lambda$sendToTracking$0_1_) -> {
         p_lambda$sendToTracking$0_1_.connection.sendPacket(p_219293_1_);
      });
   }
   ...
   public CompletableFuture<Either<IChunk, ChunkHolder.IChunkLoadingError>> func_219276_a(ChunkStatus p_219276_1_, ChunkManager p_219276_2_) {
      ...
      CompletableFuture<Either<IChunk, ChunkHolder.IChunkLoadingError>> completablefuture = (CompletableFuture)this.field_219312_g.get(i);
      if (completablefuture != null) {
         Either<IChunk, ChunkHolder.IChunkLoadingError> either = (Either)completablefuture.getNow((Either)null);
   ...
   private void chain(CompletableFuture<? extends Either<? extends IChunk, ChunkHolder.IChunkLoadingError>> p_219284_1_) {
      this.field_219315_j = this.field_219315_j.thenCombine(p_219284_1_, (p_lambda$chain$3_0_, p_lambda$chain$3_1_) -> {
         return (IChunk)p_lambda$chain$3_1_.map((p_lambda$null$1_0_) -> {
            return p_lambda$null$1_0_;
         }, (p_lambda$null$2_1_) -> {
            return p_lambda$chain$3_0_;
         });
      });
   }
   ...
   protected void func_219291_a(ChunkManager p_219291_1_) {
      ...
      if (flag5 && !flag6) {
         ...
         this.field_222983_h = UNLOADED_CHUNK_FUTURE;
         this.chain(completablefuture.thenApply((p_lambda$func_219291_a$4_1_) -> {
            p_219291_1_.getClass();
            return p_lambda$func_219291_a$4_1_.ifLeft(p_219291_1_::func_222973_a);
         }));
      }
   ...
   static {
      MISSING_CHUNK = Either.right(ChunkHolder.IChunkLoadingError.UNLOADED);
      MISSING_CHUNK_FUTURE = CompletableFuture.completedFuture(MISSING_CHUNK);
      UNLOADED_CHUNK = Either.right(ChunkHolder.IChunkLoadingError.UNLOADED);
      UNLOADED_CHUNK_FUTURE = CompletableFuture.completedFuture(UNLOADED_CHUNK);
      CHUNK_STATUS_LIST = ChunkStatus.getAll();
      LOCATION_TYPES = ChunkHolder.LocationType.values();
   }
   ...
   public static enum LocationType {
      ...
      public boolean isAtLeast(ChunkHolder.LocationType p_219065_1_) {
   ...
   public interface IListener {
      void func_219066_a(ChunkPos var1, IntSupplier var2, int var3, IntConsumer var4);
   }
