...
public class RealmsClickableScrolledSelectionListProxy extends SlotGui {
   ...
   public void renderList(int p_renderList_1_, int p_renderList_2_, int p_renderList_3_, int p_renderList_4_, float p_renderList_5_) {
      int lvt_6_1_ = this.getItemCount();

      for(int lvt_7_1_ = 0; lvt_7_1_ < lvt_6_1_; ++lvt_7_1_) {
         int lvt_8_1_ = p_renderList_2_ + lvt_7_1_ * this.itemHeight + this.headerHeight;
         int lvt_9_1_ = this.itemHeight - 4;
         if (lvt_8_1_ > this.y1 || lvt_8_1_ + lvt_9_1_ < this.y0) {
            this.updateItemPosition(lvt_7_1_, p_renderList_1_, lvt_8_1_, p_renderList_5_);
         }

         if (this.renderSelection && this.isSelectedItem(lvt_7_1_)) {
            this.renderSelected(this.width, lvt_8_1_, lvt_9_1_, Tezzelator.instance);
         }

         this.renderItem(lvt_7_1_, p_renderList_1_, lvt_8_1_, lvt_9_1_, p_renderList_3_, p_renderList_4_, p_renderList_5_);
      }
