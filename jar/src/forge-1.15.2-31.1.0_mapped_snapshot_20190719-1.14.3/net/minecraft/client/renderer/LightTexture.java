...
public class LightTexture implements AutoCloseable {
   ...
   public void updateLightmap(float p_205106_1_) {
      if (this.needsUpdate) {
         ...
         if (clientworld != null) {
            ...
            if (this.client.player.isPotionActive(Effects.NIGHT_VISION)) {
               f2 = GameRenderer.getNightVisionBrightness(this.client.player, p_205106_1_);
            ...
            for(int i = 0; i < 16; ++i) {
               for(int j = 0; j < 16; ++j) {
                  ...
                  if (clientworld.dimension.getType() == DimensionType.THE_END) {
                     ...
                     if (this.entityRenderer.getBossColorModifier(p_205106_1_) > 0.0F) {
                        f12 = this.entityRenderer.getBossColorModifier(p_205106_1_);
                        vector3f5 = vector3f1.func_229195_e_();
                        vector3f5.func_229192_b_(0.7F, 0.6F, 0.6F);
                        vector3f1.func_229190_a_(vector3f5, f12);
                     }
                  }

                  clientworld.getDimension().getLightmapColors(p_205106_1_, f, f4, f5, vector3f1);
                  vector3f1.clamp(0.0F, 1.0F);
                  float f10;
                  if (f2 > 0.0F) {
                     f10 = Math.max(vector3f1.getX(), Math.max(vector3f1.getY(), vector3f1.getZ()));
                     if (f10 < 1.0F) {
                        f12 = 1.0F / f10;
                        vector3f5 = vector3f1.func_229195_e_();
                        vector3f5.mul(f12);
