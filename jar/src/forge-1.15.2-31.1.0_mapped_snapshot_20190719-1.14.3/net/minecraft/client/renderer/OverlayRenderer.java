...
public class OverlayRenderer {
   ...
   public static void func_228734_a_(Minecraft p_228734_0_, MatrixStack p_228734_1_) {
      ...
      if (!playerentity.noClip) {
         Pair<BlockState, BlockPos> overlay = getOverlayBlock(playerentity);
         if (overlay != null && !ForgeEventFactory.renderBlockOverlay(playerentity, p_228734_1_, RenderBlockOverlayEvent.OverlayType.BLOCK, (BlockState)overlay.getLeft(), (BlockPos)overlay.getRight())) {
            func_228735_a_(p_228734_0_, p_228734_0_.getBlockRendererDispatcher().getBlockModelShapes().getTexture((BlockState)overlay.getLeft(), p_228734_0_.world, (BlockPos)overlay.getRight()), p_228734_1_);
         }
      }

      if (!p_228734_0_.player.isSpectator()) {
         if (p_228734_0_.player.areEyesInFluid(FluidTags.WATER) && !ForgeEventFactory.renderWaterOverlay(playerentity, p_228734_1_)) {
            func_228736_b_(p_228734_0_, p_228734_1_);
         }

         if (p_228734_0_.player.isBurning() && !ForgeEventFactory.renderFireOverlay(playerentity, p_228734_1_)) {
   ...
   private static BlockState func_230018_a_(PlayerEntity p_230018_0_) {
      return (BlockState)getOverlayBlock(p_230018_0_).getLeft();
   }
   ...
   private static Pair<BlockState, BlockPos> getOverlayBlock(PlayerEntity p_getOverlayBlock_0_) {
      ...
      for(int i = 0; i < 8; ++i) {
         double d0 = p_getOverlayBlock_0_.func_226277_ct_() + (double)(((float)((i >> 0) % 2) - 0.5F) * p_getOverlayBlock_0_.getWidth() * 0.8F);
         double d1 = p_getOverlayBlock_0_.func_226280_cw_() + (double)(((float)((i >> 1) % 2) - 0.5F) * 0.1F);
         double d2 = p_getOverlayBlock_0_.func_226281_cx_() + (double)(((float)((i >> 2) % 2) - 0.5F) * p_getOverlayBlock_0_.getWidth() * 0.8F);
   ...
   private static void func_228737_c_(Minecraft p_228737_0_, MatrixStack p_228737_1_) {
      ...
      for(int i = 0; i < 2; ++i) {
         ...
         float f15 = 0.5F;
         float f16 = -0.5F;
         p_228737_1_.func_227861_a_((double)((float)(-(i * 2 - 1)) * 0.24F), -0.30000001192092896D, 0.0D);
