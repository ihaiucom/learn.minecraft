...
public class ChestTileEntityRenderer<T extends TileEntity & IChestLid> extends TileEntityRenderer<T> {
   ...
   public void func_225616_a_(T p_225616_1_, float p_225616_2_, MatrixStack p_225616_3_, IRenderTypeBuffer p_225616_4_, int p_225616_5_, int p_225616_6_) {
      ...
      BlockState blockstate = flag ? p_225616_1_.getBlockState() : (BlockState)Blocks.CHEST.getDefaultState().with(ChestBlock.FACING, Direction.SOUTH);
      ChestType chesttype = blockstate.has(ChestBlock.TYPE) ? (ChestType)blockstate.get(ChestBlock.TYPE) : ChestType.SINGLE;
      ...
      if (block instanceof AbstractChestBlock) {
         ...
         p_225616_3_.func_227861_a_(-0.5D, -0.5D, -0.5D);
         TileEntityMerger.ICallbackWrapper icallbackwrapper;
         if (flag) {
            icallbackwrapper = abstractchestblock.func_225536_a_(blockstate, world, p_225616_1_.getPos(), true);
         } else {
   ...
   private void func_228871_a_(MatrixStack p_228871_1_, IVertexBuilder p_228871_2_, ModelRenderer p_228871_3_, ModelRenderer p_228871_4_, ModelRenderer p_228871_5_, float p_228871_6_, int p_228871_7_, int p_228871_8_) {
      p_228871_3_.rotateAngleX = -(p_228871_6_ * 1.5707964F);
   ...
   protected Material getMaterial(T p_getMaterial_1_, ChestType p_getMaterial_2_) {
      return Atlases.func_228771_a_(p_getMaterial_1_, p_getMaterial_2_, this.isChristmas);
   }
}
