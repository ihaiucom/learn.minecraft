...
public class DebugOverlayGui extends AbstractGui {
   private static final Map<Heightmap.Type, String> field_212923_a = (Map)Util.make(new EnumMap(Heightmap.Type.class), (p_lambda$static$0_0_) -> {
   ...
   protected List<String> call() {
      ...
      if (this.mc.isReducedDebug()) {
         return Lists.newArrayList(new String[]{"Minecraft " + SharedConstants.getVersion().getName() + " (" + this.mc.getVersion() + "/" + ClientBrandRetriever.getClientModName() + ")", this.mc.debug, s, this.mc.worldRenderer.getDebugInfoRenders(), this.mc.worldRenderer.getDebugInfoEntities(), "P: " + this.mc.particles.getStatistics() + ". T: " + this.mc.world.func_217425_f(), this.mc.world.getProviderName(), "", String.format("Chunk-relative: %d %d %d", blockpos.getX() & 15, blockpos.getY() & 15, blockpos.getZ() & 15)});
      } else {
         Entity entity = this.mc.getRenderViewEntity();
         ...
         List<String> list = Lists.newArrayList(new String[]{"Minecraft " + SharedConstants.getVersion().getName() + " (" + this.mc.getVersion() + "/" + ClientBrandRetriever.getClientModName() + ("release".equalsIgnoreCase(this.mc.getVersionType()) ? "" : "/" + this.mc.getVersionType()) + ")", this.mc.debug, s, this.mc.worldRenderer.getDebugInfoRenders(), this.mc.worldRenderer.getDebugInfoEntities(), "P: " + this.mc.particles.getStatistics() + ". T: " + this.mc.world.func_217425_f(), this.mc.world.getProviderName()});
         String s2 = this.func_223101_g();
         if (s2 != null) {
            list.add(s2);
         }

         list.add(DimensionType.getKey(this.mc.world.dimension.getType()).toString() + " FC: " + Integer.toString(((LongSet)longset).size()));
         list.add("");
               ...
               if (chunk.isEmpty()) {
                  ...
                  for(var23 = 0; var23 < var22; ++var23) {
                     ...
                     if (heightmap$type1.func_222681_b()) {
                        stringbuilder.append(" ").append((String)field_212923_a.get(heightmap$type1)).append(": ").append(chunk.getTopBlockY(heightmap$type1, blockpos.getX(), blockpos.getZ()));
                     }
                  }
         ...
         if (this.rayTraceBlock.getType() == RayTraceResult.Type.BLOCK) {
            blockpos2 = ((BlockRayTraceResult)this.rayTraceBlock).getPos();
         ...
         if (this.rayTraceFluid.getType() == RayTraceResult.Type.BLOCK) {
            blockpos2 = ((BlockRayTraceResult)this.rayTraceFluid).getPos();
   ...
   private World func_212922_g() {
      return (World)DataFixUtils.orElse(Optional.ofNullable(this.mc.getIntegratedServer()).map((p_lambda$func_212922_g$1_1_) -> {
         return p_lambda$func_212922_g$1_1_.getWorld(this.mc.world.dimension.getType());
      }), this.mc.world);
   }
   ...
   private Chunk func_212919_h() {
      if (this.field_212926_h == null) {
         ...
         if (integratedserver != null) {
            ...
            if (serverworld != null) {
               this.field_212926_h = serverworld.getChunkProvider().func_217232_b(this.field_212924_f.x, this.field_212924_f.z, ChunkStatus.FULL, false).thenApply((p_lambda$func_212919_h$4_0_) -> {
                  return (Chunk)p_lambda$func_212919_h$4_0_.map((p_lambda$null$2_0_) -> {
      ...
      return (Chunk)this.field_212926_h.getNow((Chunk)null);
   }
   ...
   protected List<String> getDebugInfoRight() {
      ...
      long l = j - k;
      List<String> list = Lists.newArrayList(new String[]{String.format("Java: %s %dbit", System.getProperty("java.version"), this.mc.isJava64bit() ? 64 : 32), String.format("Mem: % 2d%% %03d/%03dMB", l * 100L / i, bytesToMb(l), bytesToMb(i)), String.format("Allocated: % 2d%% %03dMB", j * 100L / i, bytesToMb(j)), "", String.format("CPU: %s", PlatformDescriptors.func_227775_b_()), "", String.format("Display: %dx%d (%s)", Minecraft.getInstance().func_228018_at_().getFramebufferWidth(), Minecraft.getInstance().func_228018_at_().getFramebufferHeight(), PlatformDescriptors.func_227774_a_()), PlatformDescriptors.func_227776_c_(), PlatformDescriptors.func_227777_d_()});
      if (this.mc.isReducedDebug()) {
         return list;
      } else {
         BlockPos blockpos1;
         ...
         if (this.rayTraceBlock.getType() == RayTraceResult.Type.BLOCK) {
            blockpos1 = ((BlockRayTraceResult)this.rayTraceBlock).getPos();
            BlockState blockstate = this.mc.world.getBlockState(blockpos1);
            list.add("");
            ...
            var16 = blockstate.getBlock().getTags().iterator();
         ...
         if (this.rayTraceFluid.getType() == RayTraceResult.Type.BLOCK) {
            blockpos1 = ((BlockRayTraceResult)this.rayTraceFluid).getPos();
            IFluidState ifluidstate = this.mc.world.getFluidState(blockpos1);
            list.add("");
            list.add(TextFormatting.UNDERLINE + "Targeted Fluid");
            list.add(String.valueOf(Registry.FLUID.getKey(ifluidstate.getFluid())));
            ...
            var16 = ifluidstate.getFluid().getTags().iterator();
         ...
         if (entity != null) {
            ...
            entity.getType().getTags().forEach((p_lambda$getDebugInfoRight$5_1_) -> {
   ...
   private String getPropertyString(Entry<IProperty<?>, Comparable<?>> p_211534_1_) {
   ...
   private void func_230023_a_(FrameTimer p_230023_1_, int p_230023_2_, int p_230023_3_, boolean p_230023_4_) {
      ...
      int k4;
      for(k4 = 0; k4 < j1; ++k4) {
         int k2 = (int)(along[p_230023_1_.parseIndex(lvt_8_1_ + k4)] / 1000000L);
         l1 = Math.min(l1, k2);
         i2 = Math.max(i2, k2);
         k1 += (long)k2;
      }

      k4 = this.mc.func_228018_at_().getScaledHeight();
